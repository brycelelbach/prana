...
utree<Policy>::utree (utree const& other) {
  // FIXME: Inline copy by hand here to ensure that we avoid pointless
...
utree<Policy>& utree<Policy>::copy (utree const& other, Copier copier) {
  // FIXME: Check if this is a reference; if it is, check if the reference is
...
utree<Policy>& utree<Policy>::copy (bool bool_) {
  // FIXME: Check if this is a bool; if it is, check if the bool is
  // equal to the source, and return early if it is.
  clear();
  _bool = bool_;
  _kind = bool_kind; 
...
utree<Policy>& utree<Policy>::copy (char char_) {
  // FIXME: Check if this is a symbol; if it is, check if the symbol is
  // equal to the source, and return early if it is.
  clear();
...
utree<Policy>& utree<Policy>::copy (int int_) {
  // FIXME: Check if this is a integer; if it is, check if the integer is
  // equal to the source, and return early if it is.
  clear();
  _integer = int_;
  _kind = integer_kind;
...
utree<Policy>& utree<Policy>::copy (double double_) {
  // FIXME: Check if this is a double; if it is, check if the double is
  // equal to the source, and return early if it is.
  clear();
  _floating = double_;
  _kind = floating_kind; 
...
void utree<Policy>::clear (void) {
  // FIXME: implement clear semantics
}
